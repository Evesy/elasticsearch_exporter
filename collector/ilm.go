...
type ilmStatusMetric struct {
	Type   prometheus.ValueType
	Desc   *prometheus.Desc
	Value  func(ilmStatus ILMStatusResponse, operationMode string) float64
	Labels func(operationMode string) []string
}

var (
	ilmStatuses = []string{"RUNNING", "STOPPING", "STOPPED"}
)

// Ilm information object
type Ilm struct {
	logger log.Logger
	client *http.Client
	url    *url.URL
	...
	ilmMetric       ilmMetric
	ilmStatusMetric *ilmStatusMetric
}
...
func NewIlm(logger log.Logger, client *http.Client, url *url.URL) *Ilm {
	return &Ilm{
		...
		ilmStatusMetric: &ilmStatusMetric{
			Type: prometheus.GaugeValue,
			Desc: prometheus.NewDesc(
				prometheus.BuildFQName(namespace, "ilm", "operation_mode"),
				"Operating status of ILM",
				[]string{"mode"}, nil,
			),
			Value: func(ilmStatus ILMStatusResponse, operationMode string) float64 {
				if ilmStatus.OperationMode == operationMode {
					return 1
				}
				return 0
			},
...
func (i *Ilm) fetchAndDecodeILMStatus() (ILMStatusResponse, error) {
	...
	res, err := i.client.Get(u.String())
	if err != nil {
		return isr, fmt.Errorf("failed to get ilm status from %s://%s:%s%s: %s",
			u.Scheme, u.Hostname(), u.Port(), u.Path, err)
	}
...
func (i *Ilm) Collect(ch chan<- prometheus.Metric) {
	...
	ilmStatusResp, err := i.fetchAndDecodeILMStatus()
	if err != nil {
		i.up.Set(0)
		_ = level.Warn(i.logger).Log(
	...
	for _, status := range statuses {
		ch <- prometheus.MustNewConstMetric(
			i.ilmStatusMetric.Desc,
			i.ilmStatusMetric.Type,
			i.ilmStatusMetric.Value(ilmStatusResp, status),
			status,
		)
	}
